...
WMHextraction_SkullStriping_and_FAST(){
	...
	if [ "${DARTELtemplate}" = "existing_template" ]; then
		...
		${FSLDIR}/bin/fslmaths ${subjectsDir}/cohort_probability_maps/cohort_WM_probability_map \
								-add ${subjectsDir}/cohort_probability_maps/cohort_GM_probability_map \
								-thr 0.5 \
								-bin \
								${subjectsDir}/cohort_probability_maps/DARTEL_cohort_brain_mask

		${FSLDIR}/bin/fslmaths ${subjectsDir}/${ID}/mri/preprocessing/w${T1} \
								-nan \
								-mas ${subjectsDir}/cohort_probability_maps/DARTEL_cohort_brain_mask \
								${subjectsDir}/${ID}/mri/preprocessing/nonBrainRemoved_w${T1}
